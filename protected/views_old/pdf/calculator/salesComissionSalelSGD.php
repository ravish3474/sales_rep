...
$html .= '	<span style="color:red;font-size:12px;">Last Update! : '.$date.'<span>
		...
		if($commission_status!=""){
			$condi_date .= ' AND commisson_payment_status="'.$commission_status.'" ';
				...
				$getreprot = Yii::app()->db->createCommand('select * from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND currency = "SGD" ORDER BY `date_quarter` ASC, `invoice` ASC ')->queryAll();
				...
				foreach ($getreprot as $key_report => $value_date_report) {
							...
							if($value_date_report['invoice_status'] == "Paid"){		
								...
								if($value_date_report['commisson_payment_status'] == "Paid"){
									$commissionPaymentSGD += ($value_date_report['commission'] - $value_date_report['pay_for_sales'])-$value_date_report['pay_by_credit'];
									$totalPayByCredit += $value_date_report['pay_by_credit'];
									$sumCommissionPaymaentSGD += $value_date_report['pay_for_sales']+$value_date_report['pay_by_credit'];
...
$html .= '		
	...
	<div>
		<table style="width: 100%;max-width: 100%;margin-bottom: 20px;border-spacing: 0;border-collapse: collapse;border: 1px solid #ddd;border-collapse: initial;">
			...
			$getData = Yii::app()->db->createCommand('select *  from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND currency = "SGD" ORDER BY `date_quarter` ASC, `invoice` ASC')->queryAll();
	...
	$getTLoan = Yii::app()->db->createCommand('select * from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND pay_by_customer!="0" AND currency = "SGD" order by id DESC limit 1')->queryAll();
	...
	$getLoan = Yii::app()->db->createCommand('select * from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND pay_by_customer!="0" AND currency = "SGD" order by id DESC')->queryAll();
