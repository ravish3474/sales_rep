...
<h2>Invoice - '.$_GET['year'].' year </h2>					
	<div >';
		$getDate = Yii::app()->db->createCommand('select MAX(update_date) AS datedata  from calculator where date_quarter LIKE "'.$_GET['year'].'%" ORDER BY `date_quarter` ASC, `invoice` ASC limit 1')->queryAll();
...
$html .= '	<span style="color:red;font-size:12px;">Last Update! : '.$date.'<span>
		...
		$currency = Array();
				...
				$getreprot2 = Yii::app()->db->createCommand('select * from calculator where date_quarter LIKE "'.$_GET['year'].'%" group by invoice ORDER BY `date_quarter` ASC, `invoice` ASC ')->queryAll();
				
				foreach ($getreprot2 as $key_report2 => $value_date_report2) {
					
					$getreprot = Yii::app()->db->createCommand('select * from calculator where invoice = "'.$value_date_report2['invoice'].'" limit 1 ')->queryAll();
					
					foreach ($getreprot as $key_report => $value_report) {
						
						if($value_date_report2['currency'] == "USD"){
							if($value_report['invoice_status'] == "Paid"){	
							
								$sumAmountReceivedUSD += $value_report['invoice_amount_received'];
							}
							$sumtotalUSD +=  (($value_report['total_sales']-$value_report['shipping_cost'])-$value_report['creditcard_feecost']);
						}
						if($value_date_report2['currency'] == "CAD"){
							if($value_report['invoice_status'] == "Paid"){	
								$sumAmountReceivedCAD += $value_report['invoice_amount_received'];
							}
							
							$sumtotalCAD +=  (($value_report['total_sales']-$value_report['shipping_cost'])-$value_report['creditcard_feecost']);
									
						}
						if($value_date_report2['currency'] == "SGD"){
						...
						if($value_date_report2['currency'] == "THB"){
							if($value_report['invoice_status'] == "Paid"){		
								$sumAmountReceivedTHB += $value_report['invoice_amount_received'];
							}	
							
							$sumtotalTHB +=  (($value_report['total_sales']-$value_report['shipping_cost'])-$value_report['creditcard_feecost']);
				...
				$getSale = Yii::app()->db->createCommand('select * from calculator where invoice = "'.$value_date_report2['invoice'].'" ORDER BY `date_quarter` ASC, `invoice` ASC')->queryAll();
					...
					foreach ($getSale as $key_data => $value_data) {
						...
						if($value_date_report2['currency'] == "USD"){
								...
								//$sumtotalUSD +=  (($value_data['total_sales']-$value_data['shipping_cost'])-$value_data['creditcard_feecost']);
								
								$sumCommissionPaymaentUSD += $value_data['pay_for_sales'];
							
								$totalcommissionUSD +=  $value_data['commission'];
								$payoutUSD +=  $value_data['pay_for_sales'];
						if($value_date_report2['invoice_status'] == "Paid"){
								if($value_data['commisson_payment_status'] == "Paid"){
									$commissionPaymentUSD +=  $value_data['commission'] - $value_data['pay_for_sales'];
								}else{
									$commissionPaymentUSD += $value_data['commission'];
								}	
							}
						}
						if($value_date_report2['currency'] == "CAD"){
								...
								//$sumtotalCAD +=  (($value_data['total_sales']-$value_data['shipping_cost'])-$value_data['creditcard_feecost']);
								
								$sumCommissionPaymaentCAD += $value_data['pay_for_sales'];
								
								$totalcommissionCAD +=  $value_data['commission'];
								$payoutCAD +=  $value_data['pay_for_sales'];
							if($value_date_report2['invoice_status'] == "Paid"){	
								if($value_data['commisson_payment_status'] == "Paid"){
									$commissionPaymentCAD+=  $value_data['commission'] - $value_data['pay_for_sales'];
								}else{
									$commissionPaymentCAD += $value_data['commission'];
								}
							}	
						}
						if($value_date_report2['currency'] == "SGD"){
						...
						if($value_date_report2['currency'] == "THB"){
								...
								//$sumtotalTHB +=  (($value_data['total_sales']-$value_data['shipping_cost'])-$value_data['creditcard_feecost']);
							
								$sumCommissionPaymaentTHB += $value_data['pay_for_sales'];
							
								$totalcommissionTHB +=  $value_data['commission'];
								$payoutTHB +=  $value_data['pay_for_sales'];
							if($value_date_report2['invoice_status'] == "Paid"){	
								if($value_data['commisson_payment_status'] == "Paid"){
									$commissionPaymentTHB+=  $value_data['commission'] - $value_data['pay_for_sales'];
								}else{
									$commissionPaymentTHB += $value_data['commission'];
								}
							}		
						}
					}
...
$html .= '							
	...
	$html .= '			<span>$ '.number_format($commissionPaymentUSD , 2).'</span>';
					}else{ 
	$html .= '			<span style="color:red;padding:5px 10px;">$ '.number_format($commissionPaymentUSD , 2).'</span>';
					 } 
	$html .= '		</td>';
				}
				if($sumtotalCAD != 0){  	
	...
	$html .= '			<span>$ '.number_format($commissionPaymentCAD , 2).'</span>';
					}else{ 
	$html .= '			<span style="color:red;padding:5px 10px;">$ '.number_format($commissionPaymentCAD , 2).'</span>';
					 } 
	$html .= '		</td>';
				}
				if($sumtotalSGD != 0){  
	...
	$html .= '			<span>฿ '.number_format($commissionPaymentTHB , 2).'</span>';
					}else{ 
	$html .= '			<span style="color:red;padding:5px 10px;">฿ '.number_format($commissionPaymentTHB , 2).'</span>';
					 } 
	$html .= '		</td>';
				} 	
	$html .= '		
...
$html .= '
	...
	<div>
		<table style="width: 100%;max-width: 100%;margin-bottom: 20px;border-spacing: 0;border-collapse: collapse;border: 1px solid #ddd;border-collapse: initial;">
			...
			$getData = Yii::app()->db->createCommand('select * from calculator where date_quarter LIKE "'.$_GET['year'].'%" group by invoice ORDER BY `date_quarter` ASC, `invoice` ASC')->queryAll();
	...
	$html .= '
												<span style="color:red">$ '.number_format($balance_invoice  , 2).' '.$value_res['currency'].'</span><br>';
											}else{
	$html .= '
												<span>$ '.number_format($balance_invoice  , 2).' '.$value_res['currency'].'</span><br>';
											}
										}elseif($value_res['sales_status'] == "2"){
	$html .= '													
											...
											if($balance_invoice != 0 ){
	$html .= '									<span style="color:red">$ '.number_format($balance_invoice  , 2).' '.$value_res['currency'].'</span><br>';
											}else{
	$html .= '									
												<span>$ '.number_format($balance_invoice  , 2).' '.$value_res['currency'].'</span><br>';
											}
	...
	$html .= '										
													<span style="color:red">$ '.number_format($value_res['commission'] , 2).' '.$value_res['currency'].'</span><br>';
												}else{
	$html .= '											
													<span>$ '.number_format($value_res['commission'] , 2).' '.$value_res['currency'].'</span><br>';
												}
															
											}elseif($value_res['sales_status'] == "2"){
												
	$html .= '									<span style="font-weight: bold;">R : </span>';
	...
	$html .= '					<span style="font-weight: bold;">R : </span><span> '.$value_res['commisson_payment_status'].'</span><br>';
							...
							if($value_res['commisson_payment_status'] == "Outstanding"){
	$html .= '					
								<span style="font-weight: bold;">P : </span><span style="color:red">'.$value_res['commisson_payment_status'].'</span><br>';
							}else{
	$html .= '					<span style="font-weight: bold;">P : </span><span> '.$value_res['commisson_payment_status'].'</span><br>';
							}
						}
					}		
	$html .= '									
	...
	$html .= '					
		...
		$html .= '					<span style="font-size:11px">(Date : '.$date_comm.')</span><br>';
							...
							if($value_res['commisson_payment_status'] == "Paid"){
	$html .= '					
								<span style="font-weight: bold;">P : </span><span > $ '.number_format($value_res['pay_for_sales']  , 2).' '.$value_res['currency'].'</span><br>';
	...
	$html .= '							
										<span style="color:red;padding:5px 10px;">$ '.number_format($commissionPaymentUSD , 2). ' USD</span>';
									}
	$html .= '					</td>
								<td></td>
								<td class="text-right nowrap" style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">';
...
$html .= '		
	...
	$html .= '				<tr style="background-color: #c1ffe6;">
								...
								<td  style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">
										<span>$ '.number_format($sumAmountReceivedCAD , 2).' CAD  </span>
								</td>
								<td></td>
								<td  style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">
										<span>$ '.number_format($sumtotalCAD , 2).' CAD  </span>
								</td>
								<td></td>
								<td class="text-right nowrap" style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">
										<span>$ '.number_format($totalcommissionCAD , 2).' CAD  </span>									  
								</td>
								<td class="text-right nowrap" style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">';
									if($commissionPaymentCAD == 0){	
	$html .= '							<span>$ '.number_format($commissionPaymentCAD , 2).' CAD </span>';
									}else{
	$html .= '							<span style="color:red;padding:5px 10px;">$ '.number_format($commissionPaymentCAD , 2). ' CAD </span>';
									}
	$html .= '					</td>
								<td></td>
								<td class="text-right nowrap" style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">';
									if(!empty($sumCommissionPaymaentCAD)){	
	$html .= '							<span>$ '.number_format($sumCommissionPaymaentCAD , 2).' CAD </span>';
									}
	$html .= '					</td>
...
$html .= '		
	<div>
		<table style="width: 100%;max-width: 100%;margin-bottom: 20px;border-spacing: 0;border-collapse: collapse;border: 1px solid #ddd;border-collapse: initial;">
			<thead style="display: table-header-group;vertical-align: middle;border-color: inherit;">
				<tr>
					<th colspan = "15" style="background-color:rgb(169, 185, 199);color:rgb(51, 67, 80);text-align: center;border: 1px solid #ddd;padding:8px;"> THB  </th>
				</tr>
				<tr>
	...
	$html .= '						
					...
					<td style="vertical-align: middle;text-align: left;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">';
					if($value['invoice_status'] == "Paid"){ 
	$html .= '										
						<img src="images/green.png" class="img-responsive" style="width:8px;" alt="1"> &nbsp;';
	...
	$html .= '				<tr style="background-color: #c1ffe6;">
								...
								<td  style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">
										<span>฿ '.number_format($sumAmountReceivedTHB , 2).' THB  </span>
								</td>
								<td></td>
								<td  style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">
										<span>฿ '.number_format($sumtotalTHB , 2).' THB  </span>
								</td>
								<td></td>
								<td class="text-right nowrap" style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">
										<span>฿ '.number_format($totalcommissionTHB , 2).' THB  </span>									  
								</td>
								<td class="text-right nowrap" style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">';
									if($commissionPaymentTHB == 0){	
	$html .= '							<span>฿ '.number_format($commissionPaymentTHB , 2).' THB </span>';
									}else{
	$html .= '							<span style="color:red;padding:5px 10px;">฿ '.number_format($commissionPaymentTHB , 2). ' THB </span>';
									}
	$html .= '					</td>
								<td></td>
								<td class="text-right nowrap" style="text-align: right;color: #006400;vertical-align: middle;font-weight:bold;vertical-align: middle;border: 1px solid #ddd;white-space: nowrap;padding:5px 5px;">';
									if(!empty($sumCommissionPaymaentTHB)){	
	$html .= '							<span>฿ '.number_format($sumCommissionPaymaentTHB , 2).' THB </span>';
									}
	$html .= '					</td>
