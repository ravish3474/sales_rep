...
header("Content-Disposition: attachment; filename=PG_Sales_Direct_Basketball_".date("Ymd").".xls");
header("Expires: 0");
...
$toppic = Yii::app()->db->createCommand('select *  from category where type = "Basketball" ')->queryAll();
...
<h3>Sales Direct Pricing</h3>
	<div>
		<table style="border: 1px solid #ddd;width: 100%;max-width: 100%;margin-bottom: 20px;background-color: transparent;border-spacing: 0;border-collapse: collapse;">
			<thead style="display: table-header-group;vertical-align: middle;border-color: inherit;">';
				...
				$header = Yii::app()->db->createCommand('select * from header where type = "Basketball" ')->queryAll();
	...
	$html .= 	'
			...
			<tbody>';
				$basketball = Yii::app()->db->createCommand('select *  from basketball where 1 order by sort_data ASC')->queryAll();
	...
	$html .=
		...
		<table style="border: 1px solid #ddd;width: 100%;max-width: 100%;margin-bottom: 20px;background-color: transparent;border-spacing: 0;border-collapse: collapse;">
			...
			<tbody>';
			$notes= Yii::app()->db->createCommand('select *  from notes where type = "Basketball"')->queryAll();
