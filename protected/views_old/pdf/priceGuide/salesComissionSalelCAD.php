...
$html .= '	<span style="color:red;font-size:12px;">Last Update! : '.$date.'<span>
		...
		$currency = Array();
				...
				$getreprot = Yii::app()->db->createCommand('select * from calculator where date_quarter LIKE "'.$_GET['year'].'%" AND sales_manager = "'.$sale.'" AND currency = "CAD" ORDER BY `date_quarter` ASC, `invoice` ASC ')->queryAll();
...
$html .= '		
	...
	<div>
		<table style="width: 100%;max-width: 100%;margin-bottom: 20px;border-spacing: 0;border-collapse: collapse;border: 1px solid #ddd;border-collapse: initial;">
			...
			$getData = Yii::app()->db->createCommand('select *  from calculator where sales_manager = "'.$sale.'" AND currency = "CAD" ORDER BY `date_quarter` ASC, `invoice` ASC')->queryAll();
	...
	$getTLoan = Yii::app()->db->createCommand('select * from calculator where sales_manager = "'.$sale.'" AND pay_by_customer!="0" AND date_quarter LIKE "'.$_GET['year'].'%" AND currency = "CAD" order by id DESC limit 1')->queryAll();
	...
	$getLoan = Yii::app()->db->createCommand('select * from calculator where sales_manager = "'.$sale.'" AND pay_by_customer!="0" AND date_quarter LIKE "'.$_GET['year'].'%" AND currency = "CAD" order by id DESC')->queryAll();
