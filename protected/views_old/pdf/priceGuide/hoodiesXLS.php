...
header("Content-Disposition: attachment; filename=PG_Sales_Direct_Hoodies_".date("Ymd").".xls");
header("Expires: 0");
...
$toppic = Yii::app()->db->createCommand('select *  from category where type = "Hoodies" ')->queryAll();
...
<h3>Sales Direct Pricing</h3>
	<div>
		<table style="border: 1px solid #ddd;width: 100%;max-width: 100%;margin-bottom: 20px;background-color: transparent;border-spacing: 0;border-collapse: collapse;">
			<thead style="display: table-header-group;vertical-align: middle;border-color: inherit;">';
				...
				$header = Yii::app()->db->createCommand('select * from header where type = "Hoodies" ')->queryAll();
				...
				<tr >
					<th colspan="4" style="background-color: #252c2f ;border: 1px solid #848d94 ;color: #fff;" >'.$value_header['r3c1_2'].'</th>
	...
	$html .= '			
			...
			<tbody>';
				$hoodiesDetail = Yii::app()->db->createCommand('select * from hoodies order by sort_data ASC')->queryAll();
					foreach ($hoodiesDetail as $key => $value) {
						switch ($curr) {
									case 0:
										$qty2= $value['qty2'];
										$qty3= $value['qty3'];
	...
	$html .=
		'<table style="border: 1px solid #ddd;width: 100%;max-width: 100%;margin-bottom: 20px;background-color: transparent;border-spacing: 0;border-collapse: collapse;">
			...
			<tbody>';
			$notes= Yii::app()->db->createCommand('select *  from notes where type = "Hoodies"')->queryAll();
