...
<table class="tbl_show_pguide" style="width:100%;">
	...
	<tr class="tr_head">
		...
		<th class="fix_head2" style="text-align: center;width:100%">Pattern</th>
		<th class="fix_head2" style="text-align: center;width:100%">Color</th>
		<?php
	...
	<tr>
		...
		<td style="width:60px;" class="col_backg1">
		    <?php
		    if(isset($admin_edit) && $admin_edit=="yes"){
		        if($a_item[$i]['item_image']==""){
		    ?>
                <div class="add_price" id="fabric_img_<?php echo $item_id; ?>" onclick="return adminEditUpdateImage(<?=$item_id?>,'<?=base64_encode($a_item[$i]['item_image']); ?>');" data-toggle="modal" data-target="#adminEditUpdateImage">
			...
			<?php
		          ...
		          ?>
		          <div class="add_price" id="fabric_img_<?php echo $item_id; ?>" onclick="return adminEditUpdateImage(<?=$item_id?>,'<?=base64_encode($a_item[$i]['item_image']); ?>');" data-toggle="modal" data-target="#adminEditUpdateImage">
    		          <img style="height:100px;" src="/upload/pattern/<?=$a_item[$i]['item_image']?>">
		          </div>
		      <?php
		       }
			}
