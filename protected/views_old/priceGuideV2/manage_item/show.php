	...
	if($have_group=="1" && $item_group_id!="==no_group=="){
		...
		for($j=0; $j<sizeof($a_group); $j++){
			...
			<tr class="list_item_group">
				<td colspan="4"><?php echo $a_group[$j]["group_name"]; ?></td>
			</tr>
			<?php
			$count_in_group = 1;
			for($i=0; $i<sizeof($a_item); $i++){
				...
				?>
				<tr class="list_item" id="list_item<?php echo $a_item[$i]["item_id"]; ?>">
					...
					<td style="text-align: center;">
						<button type="button" class="btn btn-warning" title="Edit info" onclick="return editProductItem(<?php echo $a_item[$i]["item_id"]; ?>);">
							<i class="fa fa-pencil"></i>
						</button>
						<button type="button" class="btn btn-danger" title="Delete item" onclick="return deleteProductItem(<?php echo $a_item[$i]["item_id"]; ?>);">
		...
		if($have_no_group){
			?>
			<tr class="list_item_group">
				<td colspan="4"><b><font style="color:#F88;">&lt; NO GROUP &gt;</font></b></td>
			</tr>
			<?php
			$count_in_group = 1;
			foreach($a_item_no_group as $tmp_key => $a_item2){
				?>
				<tr class="list_item" id="list_item<?php echo $a_item2["item_id"]; ?>">
					<td style="text-align: center;"><?php echo $count_in_group; ?></td>
					...
					<td style="text-align: center;">
						<button type="button" class="btn btn-warning" title="Edit info" onclick="return editProductItem(<?php echo $a_item2["item_id"]; ?>);">
							<i class="fa fa-pencil"></i>
						</button>
						<button type="button" class="btn btn-danger" title="Delete item" onclick="return deleteProductItem(<?php echo $a_item2["item_id"]; ?>);">
							<i class="fa fa-trash"></i>
						</button>
					</td>
				</tr>
