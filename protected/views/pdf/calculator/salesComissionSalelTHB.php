...
$html .= '	<span style="color:red;font-size:12px;">Last Update! : '.$date.'<span>
		...
		$currency = Array();
				...
				$getreprot = Yii::app()->db->createCommand('select * from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND currency = "THB" ORDER BY `date_quarter` ASC, `invoice` ASC ')->queryAll();
				...
				foreach ($getreprot as $key_report => $value_date_report) {
					...
					$pay_to_rep= $commissionPaymentTHB-$totalPayByCustomer;
...
$html .= '		
	<div>
		<table style="width: 100%;max-width: 100%;margin-bottom: 20px;border-spacing: 0;border-collapse: collapse;border: 1px solid #ddd;border-collapse: initial;">
			...
			$getData = Yii::app()->db->createCommand('select *  from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND currency = "THB" ORDER BY `date_quarter` ASC, `invoice` ASC')->queryAll();
	...
	$getTLoan = Yii::app()->db->createCommand('select * from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND pay_by_customer!="0" AND currency = "THB" order by id DESC limit 1')->queryAll();
	...
	$getLoan = Yii::app()->db->createCommand('select * from calculator where '.$condi_date.' AND sales_manager = "'.$sale.'" AND pay_by_customer!="0" AND currency = "THB" order by id DESC')->queryAll();
