...
<h2>Invoice - '.$year.' year </h2>					
	<div >';
		$getDate = Yii::app()->db->createCommand('select MAX(update_date) AS datedata  from calculator where date_quarter LIKE "'.$year.'%" AND currency = "CAD" ORDER BY `date_quarter` ASC, `invoice` ASC limit 1')->queryAll();
...
$html .= '	<span style="color:red;font-size:12px;">Last Update! : '.$date.'<span>
		...
		$currency = Array();
				...
				$getreprot2 = Yii::app()->db->createCommand('select * from calculator where date_quarter LIKE "'.$year.'%" AND currency = "CAD" group by invoice ORDER BY `date_quarter` ASC, `invoice` ASC ')->queryAll();
				
				foreach ($getreprot2 as $key_report2 => $value_date_report2) {
					
					$getreprot = Yii::app()->db->createCommand('select * from calculator where invoice = "'.$value_date_report2['invoice'].'" AND currency = "CAD" limit 1 ')->queryAll();
				...
				$getSale = Yii::app()->db->createCommand('select * from calculator where invoice = "'.$value_date_report2['invoice'].'" AND currency = "CAD" ORDER BY `date_quarter` ASC, `invoice` ASC')->queryAll();
...
$html .= '		
	...
	<div>
		<table style="width: 100%;max-width: 100%;margin-bottom: 20px;border-spacing: 0;border-collapse: collapse;border: 1px solid #ddd;border-collapse: initial;">
			...
			$getData = Yii::app()->db->createCommand('select * from calculator where date_quarter LIKE "'.$year.'%"  AND currency = "CAD" group by invoice ORDER BY `date_quarter` ASC, `invoice` ASC')->queryAll();
