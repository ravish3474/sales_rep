...
<style type="text/css">
    ...
    #viewColor .modal-dialog { width: 50%;
    }

    #viewColor .modal-dialog table td {
        text-transform: capitalize;
    } 

    #viewColor .modal-header .modal-title {
        font-size: 15px;
        color: #5E5E5E;
    }
</style>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const select = document.getElementById("cust_selector");
        if (select) {
            select.addEventListener("change", function() {
                const selectedOption = select.options[select.selectedIndex];
                document.getElementById("cust_selectorname").innerHTML =
                    selectedOption.text.replace(selectedOption.dataset.fullname, `<strong>${selectedOption.dataset.fullname}</strong>`);
            });
        }
    });
</script>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '#sel1', function() {
            ...
            $('[data-group="' + value + '"]').addClass('active');
            $('#dropdownMenu').hide();
        });
    ...
    function showPriceGuide() {
        ...
        $.ajax({
            type: "POST",
            dataType: "html",
            url: "<?php echo Yii::app()->request->baseUrl; ?>/priceGuideV2/designshowInner/product/" + prod_id + "/type/" + sat_id + "/curr/" + curr_id<?php if (isset($admin_edit) && $admin_edit == "yes") {
                                                                                                                                                        echo '+"?ade=yes"';
                                                                                                                                                    } ?>,
            success: function(resp) {
            ...
            success: function(resp) {
                ...
                $(document).ready(function() {
                    ...
                    $('#tbl_cart_info ').sortable({
                        connectWith: 'table ', // Allow dragging between both tables
                        update: function(event, ui) {
                            console.warn("Drag element work ");
    ...
    let editors = {}; // Store CKEditor instances globally
    ...
    function initializeEditors() {
        document.querySelectorAll('.ckeditor').forEach(textarea => {
            ClassicEditor
                ...
                .then(editor => {
                    editors[textarea.id] = editor; // Store the instance using the textarea ID
                })
                .catch(error => {
                    console.error(error);
                });
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    if (resp.result == "success") {
                        ...
                        const row = $(`#row_${data.extra_id}`);
                        row.find(`#extra_name${data.extra_id}`).text(data.extra_name);
