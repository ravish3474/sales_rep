...
function trimOTLdata(&$cOTLdata, $Left=true, $Right=true) {
    ...
    // Ensure 'char_data' is an array before counting its length
    $charData = isset($cOTLdata['char_data']) && is_array($cOTLdata['char_data']) ? $cOTLdata['char_data'] : [];
    $len = count($charData);
    $nLeft = 0;
    $nRight = 0;
    ...
    // Trim Left
    for($i = 0; $i < $len; $i++) {
        if($charData[$i]['uni'] == 32 || $charData[$i]['uni'] == 12288) { // 12288 = 0x3000 = CJK space
            $nLeft++;
        } else {
            break;
        }
    ...
    // Trim Right
    if ($Right && $nRight) {
        if (isset($cOTLdata['group'])) {
            $cOTLdata['group'] = substr($cOTLdata['group'], 0, strlen($cOTLdata['group']) - $nRight);
        }
        if (isset($cOTLdata['GPOSinfo']) && is_array($cOTLdata['GPOSinfo'])) {
            foreach($cOTLdata['GPOSinfo'] as $k => $val) {
                if ($k >= $len - $nRight) {
                    unset($cOTLdata['GPOSinfo'][$k]);
                }
        ...
        if (isset($cOTLdata['char_data']) && is_array($cOTLdata['char_data'])) {
            for($i = 0; $i < $nRight; $i++) {
